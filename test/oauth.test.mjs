...
import {
    InternetData, InternetDataError, OauthAccessDeniedError, OauthError, OauthExpiredTokenError,
} from '../dist/index.js';
...
test('no OAuth request carries the API key', async () => {
    ...
    const device = await client.oauth.deviceAuthorization('internetdata-cli', { scope: 'account.read' });
    await client.oauth.metadata();
    await client.oauth.exchangeDeviceCode('internetdata-cli', 'mo_dc_x');
    await client.oauth.exchangeRefreshToken('internetdata-cli', 'mo_rt_x');
    await client.oauth.revoke('internetdata-cli', 'mo_rt_x');
    assertNotError(await settle(client.oauth.pollDeviceToken('internetdata-cli', device), stub.bound));
...
test('a 2xx decodes on presence: absent stays absent, an empty scope stays present', async (t) => {
    ...
    for (const [section, operation] of Object.entries(operations)) {
        for (const c of corpus.responses[section]) {
            await t.test(`${section}: ${c.name}`, async () => {
                ...
                const args = { clientId: 'internetdata-cli', deviceCode: 'mo_dc_x' };
    ...
    for (const c of corpus.responses.revoke) {
        await t.test(`revoke: ${c.name}`, async () => {
            const stub = oauthStub([c]);
            assert.equal(await oauthClient(stub).oauth.revoke('internetdata-cli', 'mo_rt_x'), undefined);
            assert.equal(stub.requests.length, 1);
        });
    }
});
...
test('a 2xx that lacks a required member or does not parse is the ordinary error', async (t) => {
    ...
    for (const [name, reply] of Object.entries(cases)) {
        await t.test(name, async () => {
            ...
            const call = oauthClient(stub).oauth.exchangeDeviceCode('internetdata-cli', 'mo_dc_x');
...
test('aborting a poll during its first wait settles at once, with the reason', async () => {
    ...
    const outcome = await within(1500, settle(
        client.oauth.pollDeviceToken('internetdata-cli', DEVICE, { signal: controller.signal }),
        stub.bound,
    ));
...
test("the client's timeoutMs bounds every OAuth request", async () => {
    ...
    const calls = {
        ...
        'deviceAuthorization': () => client.oauth.deviceAuthorization('internetdata-cli'),
        'exchangeDeviceCode': () => client.oauth.exchangeDeviceCode('internetdata-cli', 'mo_dc_x'),
        'exchangeRefreshToken': () => client.oauth.exchangeRefreshToken('internetdata-cli', 'mo_rt_x'),
        'revoke': () => client.oauth.revoke('internetdata-cli', 'mo_rt_x'),
        'pollDeviceToken': () => client.oauth.pollDeviceToken('internetdata-cli', DEVICE),
    };
...
function oauthClient(stub, options = {}) {
    return new InternetData({ baseUrl: BASE_URL, fetch: stub.fetch, ...options });
}
...
function assertOutcome(err, want, label) {
    assert.ok(err instanceof InternetDataError, `${label}: settled with ${err}`);
