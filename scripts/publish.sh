...
# --access public is not optional for a SCOPED name: npm defaults a scoped
# package to restricted, so without it the first publish fails on a free org and
# ships a PRIVATE package on a paid one.
